...
function ContentsSection({ channel, channelNickname } : { channel : Channel, channelNickname : string}) {
    ...
    useEffect(() => {
        const onBlurSaveItem = () => {/* implementation */};
        const onVisibilityChangeSaveItem = () => {/* implementation */};
        ...
        return () => {
          ...
          document.removeEventListener("visibilitychange", onVisibilityChangeSaveItem);
        };
    }, []);
    ...
    return (
        <div>
            <div className="w-full aspect-w-16 aspect-h-9" style={{ position: 'relative', width: '100%', paddingTop: '56.25%' }}>
                <iframe
